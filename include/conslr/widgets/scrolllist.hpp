...
namespace conslr::widgets
{
    ...
    public:
        ...
        //Getters
        [[nodiscard]] constexpr const ListContainer<T>& getCurrentElement() { return IList<T>::mElements.at(mSelection); }
    ...
    ///
    template <typename T>
    inline std::pair<std::string, int32_t> constructScrollList(WidgetManager& wm, const WidgetParameterMap& params)
    {
        int priority = 0;
        ...
        auto wptr = wm.createWidget<ScrollList<T>>(priority);
        auto ptr = wptr.lock();
